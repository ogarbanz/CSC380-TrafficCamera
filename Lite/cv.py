...
def start_stream(user_email, user_speed, user_location):
    ...
    while True:
                    ...
                    if curr_time > frames[0][3]:
                        ...
                        if speed > int(user_speed):
                            ...
                            if frame_num == 0:
                                if curr_width <= 30:
                                    frame_wait = 120
                                elif 30 < curr_width <= 50:
                                    frame_wait = 100
                                elif 50 < curr_width <= 70:
                                    frame_wait = 60
        ...
        if speed_exceeded and frame_num == 0:
            ...
            saved_speed = speed
            frame_num += 1
        elif speed_exceeded and frame_num < frame_wait:
            frame_num += 1
        elif speed_exceeded and frame_num >= frame_wait:
            cv2.imwrite(date + "(2).jpg", frame)
            multiprocessing.Process(target=es.send_email, args=(user_email, saved_speed, user_location, date + "(1).jpg", date + "(2).jpg")).start()
            speed_exceeded = False
            frame_num = 0
